    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1 bg-muted/30 py-12">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-8">
            <h1 className="text-3xl font-bold">{locale === "vi" ? "Đơn hàng của tôi" : "My Orders"}</h1>
            <p className="mt-2 text-muted-foreground">
              {locale === "vi" ? "Theo dõi lịch sử mua hàng của bạn" : "View and manage your purchase history"}
            </p>
          </div>

          <Tabs value={selectedFilter} onValueChange={setSelectedFilter} className="w-full">
            <TabsList className="grid w-full max-w-md grid-cols-3">
              <TabsTrigger value="all">{locale === "vi" ? "Tất cả" : "All Orders"}</TabsTrigger>
              <TabsTrigger value="completed">{locale === "vi" ? "Hoàn tất" : "Completed"}</TabsTrigger>
              <TabsTrigger value="pending">{locale === "vi" ? "Đang xử lý" : "Pending"}</TabsTrigger>
            </TabsList>
                ...
                <Card>
                  <CardContent className="flex flex-col items-center justify-center py-12 text-center">
                    ...
                    <p className="mb-4 text-muted-foreground">
                      {locale === "vi" ? "Bạn chưa có đơn hàng nào." : "You haven't placed any orders yet."}
                    </p>
                    <Link href="/accounts">
                        ...
                        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-muted-foreground">
                              {locale === "vi" ? "Tạo ngày" : "Placed on"} {order.createdAt}
                            </p>
                            ...
                            <Dialog>
                              ...
                              <DialogContent className="max-w-2xl">
                                <DialogHeader>
                                  ...
                                  <DialogDescription>
                                    {locale === "vi" ? "Đặt ngày" : "Order placed on"} {order.createdAt}
                                  </DialogDescription>
                                </DialogHeader>
                                <div className="space-y-4">
                                  {/* Order Items */}
                                            ...
                                            {item.credentials && order.status === "completed" && (
                                              <div className="mt-2 rounded bg-muted p-2">
                                                <p className="text-xs font-medium">
                                                  {locale === "vi" ? "Thông tin tài khoản:" : "Credentials:"}
                                                </p>
                                      ...
                                      <div className="rounded-lg border p-3">
                                        <p className="text-sm">
                                          <span className="text-muted-foreground">
                                            {locale === "vi" ? "Email nhận hàng:" : "Delivery Email:"}
                                          </span>{" "}
                                          {order.deliveryEmail}
                                        </p>
                                      </div>
                                  ...
                                  <div className="flex justify-between border-t pt-3 text-lg font-bold">
                                    <span>{locale === "vi" ? "Thành tiền" : "Total"}</span>
                        ...
                        <div className="mt-4 flex flex-wrap gap-3">
                          {order.items.slice(0, 3).map((item, idx) => (
                            <div key={idx} className="flex items-center gap-2 rounded-lg border bg-muted/30 px-3 py-2">
                              <p className="text-sm">{item.name[locale]}</p>
                              {item.variant && (
                                <span className="text-xs text-muted-foreground">({item.variant[locale]})</span>
                              )}
                            </div>
                          ))}
                          {order.items.length > 3 && (
                            <div className="flex items-center rounded-lg border bg-muted/30 px-3 py-2">
                              <p className="text-sm text-muted-foreground">
                                +{order.items.length - 3} {locale === "vi" ? "sản phẩm khác" : "more"}
                              </p>
                            </div>
                          )}
                        </div>
