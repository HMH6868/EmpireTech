...
export default function CheckoutPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.fullName || !formData.email || !formData.deliveryEmail) {
      toast({
        title: locale === "vi" ? "Thiếu thông tin" : "Error",
        description: copy.requiredError[locale],
        variant: "destructive",
      })
      return
    }
    ...
    toast({
      ...
      description: copy.successDesc[locale],
    })
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="mb-8">
            ...
            >
              ...
              {copy.backToCart[locale]}
            </Link>
          </div>

          <h1 className="mb-8 text-3xl font-bold">{copy.checkout[locale]}</h1>
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
            <div className="lg:col-span-2">
              <form onSubmit={handleSubmit} className="space-y-6">
                <Card>
                  <CardHeader>
                    <CardTitle>{copy.contactInfo[locale]}</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <div>
                      <Label htmlFor="fullName">{copy.fullName[locale]}</Label>
                      <Input
                        id="fullName"
                    ...
                    <div>
                      ...
                      <Input
                        id="email"
                        type="email"
                        placeholder={locale === "vi" ? "ban@example.com" : "john@example.com"}
                ...
                <Card>
                  <CardHeader>
                    <CardTitle>{copy.deliveryTitle[locale]}</CardTitle>
                  </CardHeader>
                  <CardContent className="space-y-4">
                    <div>
                      <Label htmlFor="deliveryEmail">{copy.deliveryEmail[locale]}</Label>
                      <Input
                        id="deliveryEmail"
                        type="email"
                        placeholder={locale === "vi" ? "emailnhan@example.com" : "delivery@example.com"}
                        value={formData.deliveryEmail}
                        onChange={(e) => setFormData({ ...formData, deliveryEmail: e.target.value })}
                        required
                      />
                      <p className="mt-1 text-sm text-muted-foreground">{copy.deliveryHint[locale]}</p>
                    </div>
                    <div>
                      <Label htmlFor="deliveryPassword">{copy.preferredPassword[locale]}</Label>
                      <Input
                        id="deliveryPassword"
                        type="password"
                        placeholder={locale === "vi" ? "Nhập mật khẩu mong muốn" : "Enter preferred password"}
                        value={formData.deliveryPassword}
                        onChange={(e) => setFormData({ ...formData, deliveryPassword: e.target.value })}
                      />
                      <p className="mt-1 text-sm text-muted-foreground">{copy.preferredHint[locale]}</p>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle>{copy.paymentMethodTitle[locale]}</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <RadioGroup value={paymentMethod} onValueChange={setPaymentMethod}>
                      <div className="flex items-center space-x-3 rounded-lg border p-4 transition-colors hover:bg-muted/50">
                        ...
                        <Label htmlFor="card" className="flex flex-1 cursor-pointer items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium">{copy.card[locale]}</p>
                            <p className="text-sm text-muted-foreground">{copy.cardDesc[locale]}</p>
                          </div>
                        </Label>
                      </div>
                      <div className="mt-3 flex items-center space-x-3 rounded-lg border p-4 transition-colors hover:bg-muted/50">
                        ...
                        <Label htmlFor="paypal" className="flex flex-1 cursor-pointer items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium">{copy.paypal[locale]}</p>
                            <p className="text-sm text-muted-foreground">{copy.paypalDesc[locale]}</p>
                          </div>
                        </Label>
                      </div>
                      <div className="mt-3 flex items-center space-x-3 rounded-lg border p-4 transition-colors hover:bg-muted/50">
                        ...
                        <Label htmlFor="bank" className="flex flex-1 cursor-pointer items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium">{copy.bank[locale]}</p>
                            <p className="text-sm text-muted-foreground">{copy.bankDesc[locale]}</p>
                          </div>
                        </Label>
                      </div>
                    </RadioGroup>
                ...
                <Button type="submit" size="lg" className="w-full">
                  {copy.placeOrder[locale]} · {formatCurrency(total, { currency })}
                </Button>
              </form>
            </div>
            ...
            <div className="lg:col-span-1">
              <Card className="sticky top-20">
                <CardHeader>
                  <CardTitle>{copy.summaryTitle[locale]}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                    ...
                    {checkoutItems.map((item) => (
                      <div key={item.id} className="flex gap-3">
                        ...
                        <div className="flex-1">
                          <p className="font-medium line-clamp-1">{item.name[locale]}</p>
                          {item.variant && (
                            <p className="text-sm text-muted-foreground">{item.variant[locale]}</p>
                          )}
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">{copy.subtotal[locale]}</span>
                      <span>{formatCurrency(subtotal, { currency })}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">{copy.taxLabel[locale]}</span>
                  ...
                  <div className="flex justify-between text-lg font-bold">
                    <span>{copy.totalLabel[locale]}</span>
