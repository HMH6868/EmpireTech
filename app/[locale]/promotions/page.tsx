...
export default function PromotionsPage() {
  ...
  const handleCopyCode = (code: string) => {
    ...
    toast({
      title: copy.toastTitle[locale],
      description: copy.toastMessage[locale](code),
    })
  }
      ...
      <main className="flex-1">
        ...
        <section className="py-16">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="mb-8 flex items-center gap-3">
              ...
              <h2 className="text-2xl font-bold">{copy.activeTitle[locale]}</h2>
            </div>

            {activePromotions.length === 0 ? (
              <Card>
                <CardContent className="py-12 text-center">
                  <p className="text-muted-foreground">{copy.noActive[locale]}</p>
                </CardContent>
              </Card>
            ) : (
                    ...
                    <CardHeader>
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          ...
                          <CardTitle className="text-xl">{getLocalizedString(promo.description, locale)}</CardTitle>
                        </div>
                      </div>
                    </CardHeader>
                    <CardContent className="space-y-4">
                      <div className="rounded-lg border-2 border-dashed border-primary/30 bg-primary/5 p-4">
                        <p className="mb-2 text-xs font-medium text-muted-foreground">{copy.promoCodeLabel[locale]}</p>
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex items-center gap-2 text-muted-foreground">
                          ...
                          <span>
                            {copy.validLabel[locale]}: {promo.startDate} → {promo.endDate}
                          </span>
                        </div>
                        {promo.usageLimit && (
                          <div className="flex items-center justify-between">
                            <span className="text-muted-foreground">{copy.usageLabel[locale]}:</span>
        ...
        {upcomingPromotions.length > 0 && (
          <section className="border-t border-border/40 bg-muted/30 py-16">
            <div className="container mx-auto px-4 sm:px-6 lg:px-8">
              <div className="mb-8">
                <h2 className="text-2xl font-bold">{copy.upcomingTitle[locale]}</h2>
                <p className="mt-2 text-muted-foreground">{copy.upcomingSubtitle[locale]}</p>
              </div>
                    ...
                    <CardContent className="p-6">
                      <div className="flex items-start gap-4">
                        ...
                        <div className="flex-1">
                          ...
                          <h3 className="mb-1 font-semibold">{getLocalizedString(promo.description, locale)}</h3>
                          <p className="text-sm text-muted-foreground">
                            {locale === "vi" ? "Bắt đầu từ" : "Starts on"} {promo.startDate}
                          </p>
                        </div>
                      </div>
                    </CardContent>
        ...
        <section className="border-t border-border/40 py-16">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="mx-auto max-w-3xl">
              <h2 className="mb-8 text-center text-2xl font-bold">{copy.howToTitle[locale]}</h2>
                ...
                {copy.steps.map((step, index) => (
                  <Card key={step.title.en}>
                    <CardContent className="p-6 text-center">
                      ...
                      <h3 className="mb-2 font-semibold">{step.title[locale]}</h3>
                      <p className="text-sm text-muted-foreground">{step.description[locale]}</p>
                    </CardContent>
                  </Card>
                ))}
              </div>
